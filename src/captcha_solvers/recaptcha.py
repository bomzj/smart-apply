...
async def page_has_recaptcha(page: Page) -> bool:
...
async def find_recaptcha_with_checkbox(container: Locator) -> Locator | None:
...
async def recaptcha_already_solved(recaptcha_iframe: Locator) -> bool:
    try:
        recaptcha_response = (
            await recaptcha_iframe
            .locator('..')
...
async def solve_recaptcha(recaptcha_iframe: Locator) -> bool:
    ...
    try:
        ...
        await recaptcha_iframe.click(position={"x": 12 + 27 / 2, "y": 78 / 2})
    ...
    try:
        ...
        await wait_until(lambda: recaptcha_already_solved(recaptcha_iframe), timeout=15000)
        return True
    except:
